...
public class HtnGrounder {
	...
	public boolean isActionApplicable(Action a, State positives, State negatives) {
		...
		if (!applicable) {
			/*
			 * System.out.println(a + " not applicable in pos. state " +
			 * groundProblem.stateToString(positives) + ", neg. state " +
			 * groundProblem.stateToString(negatives)); System.out.println("  pre pos: " +
			 * groundProblem.atomSetToString(a.getPreconditionsPos()));
			 * System.out.println("  pre neg: " +
			 * groundProblem.atomSetToString(a.getPreconditionsNeg()));
			 */
		}
		return applicable;
	}
	...
	public boolean isActionNoop(Action action) {
		return action.getPreconditionsPos().getFirstTrueAtom() < 0
				&& action.getPreconditionsNeg().getFirstTrueAtom() < 0 && action.getEffectsPos().getFirstTrueAtom() < 0
		...
		for (boolean value : Arrays.asList(true, false)) {
			...
			for (int atom = 0; atom < atoms.size(); atom++) {
				...
				if (!supported) {
					...
					// groundProblem.getAtomNames().get(atom));
				}
			}
		}
