...
/**
 * Abstract base class which all grounders should inherit from. Provides a basic
 * data structure for grounding and retrieval of atoms, and implements a few
 * utility methods for different tasks during a grounding procedure.
 */
public abstract class BaseGrounder implements Grounder {

	/**
	 * Maintains all atoms, numeric atoms and derived atoms which occurred so far
	 * during grounding.
	 */
	...
	/**
	 * Converts the provided precondition (with all constant arguments) into a pair
	 * of a flat atom list and a complex precondition object. The complex
	 * precondition may be null if the condition does not contain any complex parts.
	 */
	protected Pair<List<Atom>, Precondition> splitAndGroundPrecondition(AbstractCondition cond) {
		...
		for (AbstractCondition c : simpleSet.getConditions()) {
			...
			if (liftedAtom.getPredicate().isDerived()) {
				...
				derived.setDerivedAtom(atomTable.derivedAtom(liftedAtom.getPredicate(), liftedAtom.getArguments()));
				...
				atomList.add(
						atomTable.atom(liftedAtom.getPredicate(), liftedAtom.getArguments(), liftedAtom.isNegated()));
			}
		}
	...
	/**
	 * Converts the provided effect (with all constant arguments) into a flat atom
	 * list, a list of simple conditional effects, and a complex effect object. The
	 * complex effect may be null if the condition does not contain any complex
	 * parts.
	 */
	protected Triple<List<Atom>, List<ConditionalEffect>, Effect> splitAndGroundEffect(AbstractCondition cond) {
		...
		for (AbstractCondition c : simpleSet.getConditions()) {
			...
			Condition liftedAtom = (Condition) c;
			atomList.add(atomTable.atom(liftedAtom.getPredicate(), liftedAtom.getArguments(), liftedAtom.isNegated()));
		}
		...
		while (!complexSetList.isEmpty()) {
			...
			case atomic:
				...
				atomList.add(atomTable.atom(condition.getPredicate(), condition.getArguments(), condition.isNegated()));
				break;
			case numericEffect:
			...
			case consequential:
				...
				Triple<List<Atom>, List<ConditionalEffect>, Effect> splitConsequence = splitAndGroundEffect(
						cc.getConsequence());
			...
			default:
				throw new IllegalArgumentException(
						"Condition " + c + " (type " + c.getConditionType() + ") is inside an effect.");
			}
		}
	...
	/**
	 * Splits the given condition (with all constant arguments) into two sets of
	 * conditions whereas the first set only consists of conjunctive atomic
	 * conditions and the second set contains all other (i.e. more complex)
	 * expressions.
	 */
	...
	/**
	 * Converts a lifted condition into a ground precondition. Set the 2nd argument,
	 * "negated", to "false" for a usual top-level call.
	 */
	public Precondition toPrecondition(AbstractCondition cond, boolean negated) {
		...
		case atomic:
			...
			if (c.getPredicate().isDerived()) {
				...
				pre.setAtom(atomTable.atom(c.getPredicate(), c.getArguments(), c.isNegated() != negated));
			}
			return pre;
		case negation:
	...
	protected Effect toEffect(AbstractCondition cond) {
		...
		case numericEffect:
			...
			return effect;
		default:
			error("Invalid effect type \"" + cond.getConditionType() + "\".");
			return null;
		}
	}
	...
	protected GroundNumericExpression toGroundNumExp(NumericExpression exp) {
		...
		case division:
			...
			return gExp;
		default:
			error("Invalid numeric expression type \"" + exp.getType() + "\".");
			return null;
		}
	}

	/**
	 * Simplifies non-disjunctive preconditions and effects in the operator with
	 * respect to the provided super-state. If this state is the final state a
	 * planning graph converges to, then all conditions will be removed that are
	 * constant true in the given problem ("rigid conditions").
	 */
	public Operator simplifyRigidConditions(Operator op, LiftedState liftedState) {
		...
		if (pre == null) {
			Logger.log(Logger.ERROR,
					"Precondition of " + op.toActionString() + " is trivially false after simplification.");
			System.exit(1);
		}
		if (eff == null) {
	...
	/**
	 * Simplifies away rigid conditions from the provided condition. Only operates
	 * on conjunctive conditions; disjunctive conditions are ignored and appended
	 * verbatim to the output condition.
	 * 
	 * @return null, if the condition is unsatisfiable in the state; an empty
	 *         ConditionSet instance, if the condition is constant true; a non-empty
	 *         ConditionSet instance, else
	 */
	public AbstractCondition simplifyRigidConditions(AbstractCondition condition, LiftedState liftedState,
			String context) {
		...
		for (int i = 0; i < conditions.size(); i++) {
			...
			if (c.getConditionType() == ConditionType.conjunction) {
				...
				AbstractCondition prerequisite = simplifyRigidConditions(condEff.getPrerequisite(), liftedState,
						"condeff-pre");
				AbstractCondition consequence = simplifyRigidConditions(condEff.getConsequence(), liftedState,
						"condeff-cons");
	...
	/**
	 * Assemble an Action object out of an operator whose arguments are fully
	...
	protected Action getAction(Operator liftedAction) {
		...
		// Assemble action
		Action action = new Action(actionName, pre.getLeft(), pre.getRight(), eff.getLeft(), eff.getMid(),
				eff.getRight());
	...
	protected State getInitialState(LiftedState convergedState, boolean reduceAtoms) {
			...
			if (cond.getConditionType() == ConditionType.atomic) {
				...
				if (reduceAtoms) {
					...
					if (simplified.getConditions().size() > 0) {
						...
						initialStateAtoms.add(atomTable.atom(c.getPredicate(), c.getArguments(), c.isNegated()));
					}
		...
		initialStateAtoms.add(atomTable.atom(trueCondition.getPredicate(), trueCondition.getArguments(), false));
		...
		for (Function f : problem.getInitialFunctionValues().keySet()) {
			NumericAtom atom = atomTable.numericAtom(f, problem.getInitialFunctionValues().get(f));
	...
	/**
	 * Compiles all atom names into a flat list, where the ith item is the name of
	 * the atom of ID i, and returns the list.
	 */
	...
	/**
	 * Assembles the logical meaning of all relevant derived atoms and grounds the
