...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;; Version 2 , Fonctionnelle, lergement plus performante que la V1 
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;------------------------------------------------------------------
    ...
    (:method do_cocktail_in_shot
        :parameters (?sh - shot ?c - cocktail)
        :expansion  (
                        (tag t1 (get_first_ingredient ?c ?h))
                        (tag t2 (get_second_ingredient ?c ?h))
                        (tag t3 (serve_shot ?sh ?c))
                    )
        :constraints( 
                    and (before (and (ontable ?sh) (clean ?sh) (handempty ?h)) t1)
                    )
    )
...
;------------------------------------------------------------------
    ...
    (:method get_first_ingredient 
        ...
        :expansion  (
                        (tag t1 (do_fill_shot ?sh ?i ?h1))
                        (tag t2 (pour-shot-to-clean-shaker ?sh ?i ?s ?h1 ?l0 ?l1))
                        (tag t3 (clean-shot ?sh ?i ?h1 ?h2))
                    )
        :constraints(
                    and
                        (before (and (cocktail-part1 ?c ?i) ( empty ?sh)) t1)
                    )
    )
    ...
    (:method get_second_ingredient 
        :parameters (?c - cocktail ?h1 - hand)
        :expansion  (
                        (tag t1 (fill-shot ?sh ?i ?h1 ?h2 ?d))
                        (tag t2 (pour-shot-to-used-shaker ?sh ?i ?s ?h1 ?l0 ?l1))
                        (tag t3 (leave ?h1 ?sh))
                    )
        :constraints(
                    and
                        (before (and (cocktail-part2 ?c ?i) (holding ?h1 ?sh)) t1)
                    )
    )
...
;------------------------------------------------------------------
    ...
    (:method serve_shot
        ...
        :expansion  (
                        ...
                        (tag t1 (do_clean_shot ?sh ?b ?h))
                        (tag t2 (grasp ?h1 ?s))
                        (tag t3 (shake ?c ?i1 ?i2 ?s ?h1 ?h))
                        (tag t4 (pour-shaker-to-shot ?c ?sh ?h1 ?s ?l2 ?l1))
                        (tag t5 (empty-shaker ?h1 ?s ?c ?l1 ?l0))
                        (tag t6 (clean-shaker ?h1 ?h ?s))
                        (tag t7 (leave ?h1 ?s))
        ...
        :constraints(
                    and
                        (before (and (cocktail-part1 ?c ?i1) (cocktail-part2 ?c ?i2)) t1)
                    )
...
;------------------------------------------------------------------
    ...
    (:method do_clean_shot
        ...
        :constraints(
                    and 
                        (before (and (not(clean ?s)) (not(holding ?h1 ?s))) t1)
                    )
    )
...
;------------------------------------------------------------------
; 3 cas :
;    j'ai un shot sale de i
;    sinon j'ai un shot propre
;    sinon j'ai un shot sale que je lave
;
    ...
    (:method do_fill_shot
        ...
        :expansion  (
                        (tag t1 (grasp ?h1 ?sh))
                        (tag t2 (refill-shot ?sh ?i ?h1 ?h2 ?d))
                    )
        :constraints(
                    and
                        (before (and (used ?sh ?i) (dispenses ?d ?i)) t1)
                    )
    ...
    (:method do_fill_shot
        ...
        :expansion  (
                        (tag t1 (grasp ?h1 ?sh))
                        (tag t3 (fill-shot ?sh ?i ?h1 ?h2 ?d))
                    )
        :constraints(
                    and
                        (before (and (clean ?sh) (dispenses ?d ?i)) t1)
                    )
    ...
    (:method do_fill_shot
        ...
        :expansion  (
                        (tag t1 (grasp ?h1 ?sh))
                        (tag t2 (clean-shot ?sh ?b ?h1 ?h2))
                        (tag t3 (fill-shot ?sh ?i ?h1 ?h2 ?d))
                    )
        :constraints(
                    and
                        (before (and (not (used ?sh ?i)) (not (clean ?sh)) (used ?sh ?b) (dispenses ?d ?i)) t1)
                    )
